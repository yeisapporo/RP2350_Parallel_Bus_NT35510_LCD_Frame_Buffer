...
// drawing test
...
#if 0
    for(int i = 0; i < 8; i++) {
        lcd.line(384 + random(416), random(480), 384 + random(416), random(480), lcd.rgb(random(128), random(128), random(128)), true);
        lcd.box(384 + random(416), random(480), 384 + random(416), random(480), lcd.rgb(random(128), random(128), random(128)), true);
        lcd.circle(384 + random(416), random(480), random(480), lcd.rgb(random(128), random(128), random(128)), true);
    }
#endif
    ...
    for(int y = 0; y < 16; y++) {
        for(int x = 0; x < 32; x++) {
            ...
            if(!first_draw_done || (old_table_y != table_y || old_table_x != table_x)) {
                lcd.bitblt(chip_map, x * 24 + 16, y * 24 + 16, table_x, table_y, 24, 24, false);
            }
        }
    }
#endif
    ...
    shiftOut(HC595_SI, HC595_SCK, MSBFIRST, 0xaa);
    shiftOut(HC595_SI, HC595_SCK, MSBFIRST, 0xff);
